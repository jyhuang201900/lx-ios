...
export const hapticFeedback = (style: 'light' | 'medium' | 'heavy' | 'success' | 'warning' | 'error' = 'light') => {
  if (Platform.OS == 'ios' && typeof UtilsModule?.hapticFeedback == 'function') UtilsModule.hapticFeedback(style)
}

export const showNativeToast = (message: string, duration: 'short' | 'long' = 'short', position: 'top' | 'center' | 'bottom' = 'bottom') => {
  if (Platform.OS != 'ios' || typeof UtilsModule?.showToast != 'function') return false
  UtilsModule.showToast(message, duration == 'long' ? 3200 : 1800, position)
  return true
}
