...
import { getSystemLocales, isIgnoringBatteryOptimization, isNotificationsEnabled, requestNotificationPermission, requestIgnoreBatteryOptimization, shareText, showNativeToast } from '@/utils/nativeModules/utils'
...
export const toast = (message: string, duration: 'long' | 'short' = 'short', position: 'top' | 'center' | 'bottom' = 'bottom') => {
  if (!isAndroid) {
    if (!showNativeToast(message, duration, position)) console.log(`[toast:${duration}:${position}] ${message}`)
    return
  }
  let _duration
  switch (duration) {
...
export const cheatTip = async() => {
  ...
  return tipDialog({
    title: '谨防被骗提示',
    message: `1. 本软件无微信公众号之类的所谓「官方账号」，也不会以任何形式收费，谨防被骗！\n
2. 本软件完全无广告且无引流（如需要加群、关注公众号之类才能使用或者升级）的行为，若你使用过程中遇到广告或者引流的信息，则表明你当前运行的软件被人修改过，请注意甄别。`,
    btnText: '我知道了 (Close)',
    bgClose: true,
  }).then(() => {
