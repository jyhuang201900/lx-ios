...
RCT_EXPORT_METHOD(hapticFeedback:(NSString *)style) {
  dispatch_async(dispatch_get_main_queue(), ^{
    if ([style isEqualToString:@"success"] || [style isEqualToString:@"warning"] || [style isEqualToString:@"error"]) {
      UINotificationFeedbackGenerator *generator = [[UINotificationFeedbackGenerator alloc] init];
      [generator prepare];
      UINotificationFeedbackType type = [style isEqualToString:@"success"] ? UINotificationFeedbackTypeSuccess : ([style isEqualToString:@"warning"] ? UINotificationFeedbackTypeWarning : UINotificationFeedbackTypeError);
      [generator notificationOccurred:type];
      return;
    }
    UIImpactFeedbackStyle impactStyle = [style isEqualToString:@"heavy"] ? UIImpactFeedbackStyleHeavy : ([style isEqualToString:@"medium"] ? UIImpactFeedbackStyleMedium : UIImpactFeedbackStyleLight);
...
RCT_EXPORT_METHOD(showToast:(NSString *)message duration:(nonnull NSNumber *)duration position:(NSString *)position) {
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    if (window == nil || message.length == 0) return;
    ...
    NSLayoutConstraint *vertical = [position isEqualToString:@"top"]
      ? [label.topAnchor constraintEqualToAnchor:window.safeAreaLayoutGuide.topAnchor constant:18]
      : ([position isEqualToString:@"center"]
        ? [label.centerYAnchor constraintEqualToAnchor:window.centerYAnchor]
        : [label.bottomAnchor constraintEqualToAnchor:window.safeAreaLayoutGuide.bottomAnchor constant:-22]);
    [NSLayoutConstraint activateConstraints:@[
      ...
      [label.leadingAnchor constraintGreaterThanOrEqualToAnchor:window.leadingAnchor constant:24],
      [label.trailingAnchor constraintLessThanOrEqualToAnchor:window.trailingAnchor constant:-24],
      vertical,
    ]];
    [UIView animateWithDuration:0.2 animations:^{ label.alpha = 1; }];
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(MAX(600, duration.integerValue) * NSEC_PER_MSEC)), dispatch_get_main_queue(), ^{
      [UIView animateWithDuration:0.25 animations:^{ label.alpha = 0; } completion:^(__unused BOOL finished) { [label removeFromSuperview]; }];
    });
  });
}
